...
use sp_runtime::traits::{AtLeast32Bit, Bounded, Member}; //traits::Member;
...
impl<Storage, Key, Value> LinkedList<Storage,Key,Value>
 where // 为参数指定具体的约束条件 必须要实现了指定的特征的参数
  // Option作用 统一处理参数为空的情况，rust会给一个默认值
 Storage: StorageMap<(Key,Option<Value>),LinkedItem<Value>,Query = Option<LinkedItem<Value>>>,
 Value: Parameter + Member + AtLeast32Bit + Bounded + Default + Copy,//Parameter+Member+Copy, // copy clone
 Key: Parameter,
